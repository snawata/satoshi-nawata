...
$J^\mu = (\rho, \bm j) = (q \delta^3(\bm r-\bm r(t)), \partial_t \bm r(t) \rho )$
...
\begin{align}
 &Z_\mathrm{F}(\tau) = Z_\mathrm{NS}^{+} +Z_\mathrm{NS}^{-} +e^{i\theta} Z_\mathrm{R}^{+} +e^{i\theta} Z_\mathrm{R}^{-} \ , \label{eq:1}\\
...
\begin{align}
 |\bm s ; k \rangle = | s_0, s_1, s_2, s_3, s_4 ; k \rangle \ ,
\end{align}
where $s_i =\pm \frac{1}{2}$.
Choice for $s_0$ is a choice of $\bm{16}$ or $\bm{16'}$, and we chose $s_0=+\frac{1}{2}$
required by BRST quantization (Dirac equation).
The other choices of $s_i \ (i \neq 0)$ simply give degeneracies of $2^4 = 16$.
Note that $(-1)^F |\bm s ; k \rangle = (-1)^{\bm 1 \cdot (\bm 1-2\bm s )/2 }|\bm s ; k \rangle$,
where $\bm 1 \cdot (\bm 1 -2\bm s )/2 = \sum_{i=0}^4 \left(\frac{1}{2} -s_i\right)$.
Therefore,
\begin{align}
 \langle \bm s; k| (\pm 1)^F |\bm s ; k \rangle =
 \begin{cases}

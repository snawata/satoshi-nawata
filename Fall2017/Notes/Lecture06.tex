...
\begin{align}
 ...
 \left(P\cdot \epsilon\right)^{ab} +2\wt\omega h^{ab} \right] \nonumber\\
...
\begin{align}
 &\partial \ol\epsilon = \ol\partial \epsilon = 0 \ , \\
...
For sphere we have
\begin{align}
 &\delta^\perp h_{zz} = \delta^\perp h_{\ol z\ol z} = 0 \ , \\
 &\epsilon = a_0 +a_1 z + a_2 z^2 \ , \\
...
\begin{align}
 ...
 \prod_{j=1}^n g_\textrm{st} \sqrt h V_j  \nonumber\\
...
(For the interested students see \cite[\S4]{Polchinski} Vol.1 Sec. 6.2.)
...
\begin{align}
 &X^\mu = \sum_I x_I^\mu X_I \ ,  \\
 &\Delta X_I = -\omega_I^2 X_I \ ,  \\
...
So the result is
\begin{align}
 Z[J] &= C (2\pi)^D \delta^D(J_0) \prod_{I\neq 0}\exp \left( -\frac{\pi\alpha' J_I\cdot J_I}{\omega_I} \right)  \nonumber\\
...
where
\begin{align}
 &B(a,b,c) = \int d^2z |z|^{2a-2} |1-z|^{2b-2} = \pi \frac{\Gamma(a)\Gamma(b)\Gamma(c)}{\Gamma(a+b)\Gamma(b+c)\Gamma(c+a)} \ , \\
 &s = -k_{1+2}^2 = -k_{3+4}^2 = -2 k_1\cdot k_2 -\frac{8}{\alpha'} \ , \nonumber\\
 &t = -k_{1+3}^2 = -k_{2+4}^2 = -2 k_1\cdot k_3 -\frac{8}{\alpha'} \ , \\
...
\begin{align}
 A_{0,3} &= a_3 (2\pi)^D \delta^D (\sum k_i) \ , \qquad a_3 \simeq C g_\textrm{st} \ , \\

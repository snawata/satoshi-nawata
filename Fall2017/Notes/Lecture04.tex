...
%  &\alpha^\mu_{-n} |0;k\rangle  \ra   \partial^n X^\mu e^{ik\cdot X}  \\
...
\begin{align}
 A_n = \sum_g \int \left(\DD h_{ab}\right)_{g,0} \int \DD X^\mu e^{-S_\sigma [X^\mu,h_{ab}]} \prod_{i=1}^n \int d^2z \sqrt h V_i  \ .
\end{align}
...
\begin{align}
 &0 = \nabla^z T_{zz} +\nabla^{\ol z} T_{\ol zz} = \nabla_{\ol z} T_{zz} +\nabla_{z} T_{\ol zz}
 = \ol\partial \left( T_{zz} -2a\left( \partial\partial\Omega -\partial\Omega\partial\Omega \right) \right)  \\
...
For example,
\begin{align}
 &\nabla^a T_{az} = \nabla^z T_{zz} +\nabla^{\ol z} T_{\ol zz} = \ol\partial T(z) = 0  \\
...
Especially,
\begin{align}
 \delta_\epsilon T(z) = \epsilon(w)\partial T(z) +2\partial\epsilon(z) T(z) +\frac{c}{12} \partial^3 \epsilon(z) \ .
\end{align}
...
\begin{align}
 &z \to \wt z = z -\epsilon(z) \ , \\
 &T_{zz}(z) \to \wt T_{\wt z\wt z}(\wt z) = \left( \partial_{z} \wt z \right)^{-2} T_{zz}(z) \ , \\
...
\begin{align}
 G_{\mu\nu}(X) = \eta_{\mu\nu} + f_{\mu\nu} (X) \ .
\end{align}
Then, partition function becomes
\begin{align}
...
Discussion here is a simpler version of Hosomichi's note(Lecture 03).
Note that discussion on this subject in textbooks(Green-Schwarz-Witten, Polchinski, Hosomichi etc.) are incomplete.
If you are interested in the details you should consult \cite{Callan:1988xx},
which is a nice review from TASI lecture 1988.

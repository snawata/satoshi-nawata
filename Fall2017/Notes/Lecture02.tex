...
See Fig.~\ref{fig1.eps}, which is a naive but appropriate extension from quantum field theory (QFT) interaction to string interaction.
\begin{figure}[htb]
 \centerline{\includegraphics[width=250pt]{fig1.eps}}
...
Given that we adopt the operator expression the string interaction is written as in Fig.~\ref{fig2.eps} as well as
\begin{align}
...
\begin{figure}[htb]
 \centerline{\includegraphics[width=250pt]{fig2.eps}}
...
inserted at the origin, which is called vertex operator (see Fig.~\ref{fig3.eps}).
\begin{figure}[htb]
 \centerline{\includegraphics[width=250pt]{fig3.eps}}
...
We will learn operator analysis of 2d conformal field theory (CFT), including OPE, Ward identity etc.,
...
\begin{align}
 X^\mu(z,\ol z) &= X_R^\mu (z) +X_L^\mu (\ol z)   ,   \\
...
\begin{align}
 ...
 = \langle \frac{1}{\pi\alpha'}\partial_z\partial_{\ol z} X^\mu (z) X^\nu(w) + \eta^{\mu\nu}\delta^2(z-w) \rangle \ .
\end{align}
Therefore,
\begin{align}
 \partial\ol\partial X^\mu (z) X^\nu(w) = -\pi\alpha' \eta^{\mu\nu}\delta^2(z-w) \ ,
\end{align}
...
% \end{align}
\begin{align}
 \nord{f[X]} = \exp \left[ -\frac{1}{2} \int d^2z d^2w\ G(z,w) \frac{\delta}{\delta X^\mu(z)} \frac{\delta}{\delta X_\mu(w)} \right]
 f[X] \ .
\end{align}
For example,
\begin{align}
...
\begin{align}
 \nord{f[X]} \nord{g[X]} = \exp \left[ \int d^2z d^2w\ G(z,w) \left. \frac{\delta}{\delta X^\mu(z)} \right|_{f} \left. \frac{\delta}{\delta X_\mu(w)} \right|_{g} \right] \nord{fg[X]} \ .
\end{align}
Example 1:
\begin{align}
 \nord{ \partial X(z)} \nord{ X(w)} = \nord{\partial X(z) X (w)} +\partial_z G(z,w)  
 = \nord{\partial X (z) X (w)} -\frac{\alpha'}{2} \frac{1}{z-w} \ .
\end{align}
Example 2:
\begin{align}
 \nord{e^{ik\cdot X(z)}} \nord{e^{ik'\cdot X(w)}} &= \exp\left[ (ik) \cdot (ik') \left( -\frac{\alpha'}{2} \log|z-w|^2 \right)\right]
 \nord{e^{ik\cdot X(z) +ik'\cdot X(w)}} \nn &= |z-w|^{\alpha'k\cdot k'} \nord{e^{ik\cdot X(z) +ik'\cdot X(w)}} \ .
\end{align}
...
\subsection{Noether's theorem and Ward identities}
...
If we promote the parameter to be WS coordinate dependent (ie $\wh \delta X = \epsilon(z,\ol z) (\cdots)$), then, $\wh \delta S$ is no longer zero but has to take the following form.
\begin{align}
...
Suppose we want to have the transformation of operator $f[X](w,\ol w)$ under $\delta$.
We set
\begin{align}
 \epsilon(z,\ol z) =
 \begin{cases}
  \epsilon \quad (\textrm{const.}) \quad &\textrm{for}\ z \in D \ ,  \\
  0                                      &\textrm{for}\ z \not\in D \ , \\
 \end{cases}
\end{align}
where $D$ is a disk containing $w$. Then,
\begin{align}
 0 = \int \DD X \ \wh \delta \left[ e^{-S} f(w) \right]
 = \int \DD X e^{-S}\  \left[ \delta f(w) -\wh \delta S \cdot f(w) \right]  .
\end{align}
Therefore,
\begin{align}
 \delta f(w,\ol w) &= \int \frac{d^2z}{2\pi} \epsilon(z, \ol z)
 \left( \partial \ol j(\ol z) +\ol\partial j(z) \right) f(w, \ol w)  \nn
 &= \frac{\epsilon}{2\pi i} \oint_{\partial D} \Big( dz\ j(z) -d\ol z\ \ol j(\ol z) \Big) f(w,\ol w) \ ,
\end{align}
This is \textbf{Ward identity}.


Let us see an example.
\begin{align}
 &S = \frac{1}{2\pi \alpha'} \int d^2z\ \partial X \cdot \ol\partial X \ ,  \qquad
 \delta X = \epsilon g(z)  \quad (\textrm{holomorphic})  \\
 \ra \quad &\wh \delta S = \frac{-2}{2\pi \alpha'} \int d^2z\ \epsilon(z,\ol z) \ol\partial \left( g \partial X\right)  \\
...
% \subsubsection*{Ward identity for translation}
...
% Therefore,
% \begin{align}
%  &T \equiv T_{zz} = -\frac{1}{\alpha'} \partial X \partial X \ , \\
%  &\ol T \equiv T_{\ol z\ol z} = -\frac{1}{\alpha'} \ol\partial X \ol\partial X \ , \\
%  &T_{z\ol z} = 0 \ .
% \end{align}

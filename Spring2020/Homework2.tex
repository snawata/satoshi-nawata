...
\centerline{\Large \bf Homework 2: Due at class on March 23}



\section{Renormalization of $\phi^{3}$-theory}
Let us consider $\phi^{3}$-theory in 6d spacetime with the Lagrangian density
$$
\mathcal{L}=\frac{1}{2}(\partial \phi)^{2}-\frac{m^{2}}{2} \phi^{2}-\frac{g}{3 !} \phi^{3}
$$
\begin{enumerate}
\item Determine the superficial divergent amplitudes. Write the renormalized Lagrangian density and derive the Feynman rules.
\item  Calculate the propagator correction at one-loop order and determine $\delta_Z$ and $\delta_m$.
\item Calculate the vertex correction and find $\delta_g$.
\end{enumerate}



\section{$\beta,\gamma$ for Yukawa theory}
Let us consider the pseudoscalar Yukawa Lagrangian
$$
\mathcal{L}=\frac{1}{2}\left(\partial_{\mu} \phi\right)^{2}-\frac{1}{2} m^{2} \phi^{2}+\bar{\psi}(i \slashed \partial-M) \psi-i g \bar{\psi} \gamma^{5} \psi \phi-\frac{\lambda}{4!}\phi^4
$$
where $\phi$ is a real scalar field and $\psi$ is a Dirac fermion.
\subsection{}
Compute the one-loop contributions to  $\beta_m, \beta_M$ and
$\gamma_\psi, \gamma_\phi$.
\subsection{}
Compute the Callan-Symanzik $\beta$ functions for $\lambda$ and $g$ :
$$
\beta_{\lambda}(\lambda, g), \quad \beta_{g}(\lambda, g)
$$
to leading order in coupling constants, assuming that $\lambda$ and $g^{2}$ are of the same order. Sketch the coupling constant flows in the $\lambda$-$g$ plane.

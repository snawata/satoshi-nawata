...
\centerline{\Large \bf Homework 4: Due at class on April 20}





\section{Tree-level amplitude in QCD}

Consider quark-antiquark pair production in QCD  with gauge group $G$, specifically
$u\bar u\to d\bar d$.
There is only one tree diagram contributing to this process,
\begin{figure}[h]\centering
    \includegraphics[width=4cm]{uudd}
\end{figure}
Evaluate this diagram, then sum/average the $|{\cal M}|^2$
over both spins and {\it colors} of the final/initial particles to
calculate the total cross section by neglecting the quark masses. You can use the result of $e^-e^+\to \mu^-\mu^+$.




\section{Tree-level amplitude in scalar QCD}

Let us consider scalar QCD with gauge group $G$ and
complex scalar fields $\Phi^i(x)$ in some representation $r$ of $G$.

\subsection{}
Write down the Lagrangian of scalar QCD and extract Feynman rules.

\subsection{}
We consider tree-level annihilation of a scalar `quark' $\Phi^i$
and an `antiquark' $\Phi^*_j$ into a pair of gauge bosons
with adjoint colors $a$ and $b$:  $\Phi+\Phi^*\to 2$~gauge bosons.
At the tree level, there are four Feynman diagrams contributing
to this process.
Draw the diagrams and write down the tree-level annihilation amplitude.


\subsection{}
Rewrite it in the form
\begin{equation}
\mathcal{M}(j+i \rightarrow a+b)=F \times\left\{t_r^{a}, t_r^{b}\right\}+i G \times\left[t_r^{a}, t_r^{b}\right]
\end{equation}
where $F$ and $G$ are some functions of all the momenta and
of the two vectors' polarizations. Give explicit formulae for $F$ and $G$.

\subsection{}
Next, let us sum the $|{\cal M}|^2$ over the gauge boson's colors
and average over the scalars' colors.
Show that
\begin{equation}
\frac{1}{d(r)^2} \sum_{i j} \sum_{a b}|\mathcal{M}|^{2}=\frac{C_2(r)}{d(r)} \times\left((4 C_2(r)-C_2(\operatorname{adj})) \times|F|^{2}+C_2(\mathrm{adj}) \times|G|^{2}\right)
\end{equation}
In particular, for scalars in the fundamental representation
of the $\SU(N)$ gauge group,
\begin{equation}
\frac{1}{N^{2}} \sum_{i j} \sum_{a b}|\mathcal{M}|^{2}=\frac{N^{2}-1}{2 N^{2}}\left(\frac{N^{2}-2}{N} \times|F|^{2}+N \times|G|^{2}\right)
\end{equation}



\subsection{Bonus problem}
Evaluate $F$ and $G$ in the center of mass frame, where the gluon polarizations are purely spatial and transverse to the gluon momenta $\pm \mathbf{k}$. Combining all the results and calculate the cross-section for the annihilation process.
...
\section{$\beta$-function in QCD}
\subsection{}
Let us consider QCD with gauge group $G$ and $n_{f}$ species of fermions in a representation $r$. The beta function of the gauge coupling constant is given by
$$\beta(g)=g  \frac{\partial}{\partial \log \mu}\left(-\delta_{1}+\delta_{2}+\frac{1}{2} \delta_{3}\right)~.$$
The derivation of $\delta_3$ is schematically given in the lecture. Fill out the detail of the computation of $\delta_3$, and calculate $\delta_1$ and $\delta_2$. Show that
$$\beta(g)=-\frac{g^{3}}{(4 \pi)^{2}}\left[\frac{11}{3} C_{2}(G)-\frac{4}{3} n_{f} C(r)\right]~.$$

\subsection{}
Calculate the  $\delta_2^c$ and $\delta_1^c$ counterterms for the ghosts fields. Show  that the difference $\delta_1^c-\delta_2^c$
for the ghosts is exactly the same as the $\delta_1-\delta_2$ difference for the quarks.

...
\centerline{\Large \bf Homework 1: Due at class on Sep 23}



\subsection*{1}
The following figure is the phase diagram of H${}_2$O. Where can you see a first-order and second-order phase transition? In addition, what is the order parameter?
...
\subsection*{2}
\begin{itemize}
  \item Derive the infinitesimal  transformation (2.20) from the finite transformation (2.21) for SCT.
    \item Derive the conformal factor (2.22) of  SCT.
\item Using either explicit forms of the generators in Table 1 or (2.23), derive (2.24) in the lecture note.
\end{itemize}

\subsection*{3. Bonus problem (3pt)}
The notations are the same as (1.3) and (1.4) of the lecture note.
\subsubsection*{3.1}
Suppose that the free energy density $f:=F/N$ obeys
$$
f(t,h)=b^{-d} f(b^{y_t}t,b^{y_h}h)~,
$$
under the scaling transformation $x\to x/b$.
Show that the critical exponents can be expressed
\bea\nonumber
\a&=2-d/y_t\cr
\b&=(d-y_h)/y_t\cr
\g&=(2y_h-d)/y_t\cr
...
\eea
\subsubsection*{3.2}
Under the scaling transformation $x\to x/b$, the correlation length $\xi$ should behave as $\xi\to \xi/b$. Show that $\nu=1/y_t$.
Using the fact that the susceptibility can be written in terms of the two-point correlation function of $\sigma$
$$
\chi=\frac1T\int d^d\mathbf{r}   ~G(\mathbf{r})~,
$$
express $\a,\b,\g,\d$ in terms of $\nu,\eta$.

...
\centerline{\Large \bf Homework 2: Due at class on March 18}



\vspace{.5cm}
\noindent \textbf{1}.  Show that an open ball $B_n=\{x\in \bR^n| ~|x|<1\}$ is diffeomorphic to $\bR^n$ by constructing a smooth bijection map.
...
\noindent \textbf{3}.  Write down vector fields that generate the rotation along $x$-, $y$-, $z$-axis in $\bR^3$. Find the commutation relations of these vector fields. Compare the theory of angular momenta in quantum mechanics.
...
\vspace{.5cm}
\noindent \textbf{4}. Let $e$ be the identity element of $SO(3)$. Show that the tangent space $T_e SO(3)$ at $e$ is spanned by tangent vectors of curves in $SO(3)$
$$
...
\vspace{.5cm}
\noindent \textbf{5}. Show that the tangent space $T_e SU(2)$ is spanned by $i\sigma_x$, $i\sigma_y$ and $i\sigma_z$ (the Pauli matrices by $i$).

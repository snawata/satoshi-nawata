...
\be \label{deRham-cohomology}
H^k_{dR}(M)=Z^k(M)/B^k(M)~,
\ee
and its dimension is called
the \(k\)-th \textbf{Betti number} of \(M\).
In other words, the de Rham cohomology group of $M$ is the cohomology group of \textbf{de Rham complex} $(\Omega^\bullet,d)$
\be \label{deRham-complex}
...
|X_p|=\sqrt{g(X_p,X_p)}~.
$$
For a curve $\gamma:[a,b]\to M$, the length $L(\gamma)$ of a curve can be defined by
$$
L(\gamma)=\int_{a}^b |\dot \gamma(t)|dt=\int_a^bdt \sqrt{g_{ij}\frac{dx^i}{dt}\frac{dx^j}{dt}}~.
$$
Therefore, very roughly speaking, Riemannian geometry studies a geometry in which the ``length'' of a one-dimensional curve $\gamma$ is defined.
...
$$\textrm{vol} = \sqrt{g}dx^1 \wedge dx^2 \wedge \cdots \wedge dx^n~,$$
...
one can show that a necessary and sufficient condition of harmonic form:
\be\label{harmonic-deRham}\Delta\omega=0\quad \textrm{iff} \quad d\omega=0=\delta\omega
~.\ee
...
For an oriented compact Riemannian manifold, we have the orthogonal decomposition
$$
...
\bthm[Hodge theorem]
On an oriented compact Riemannian manifold,  the natural map $\bH^k(M)\to H^k_{dR}(M)$ is an isomorphism.
\ethm
...
The Hodge theorem tells us that for a nontrivial $\omega\in H_{dR}^k(M)$, one can choose a harmonic $k$-form as a representative. Due to $\Delta\ast =\ast \Delta$, $\eta=\ast \omega$ is a harmonic $(n-k)$-form. Since $\omega\neq 0$, we have
$$
\int_M\omega\wedge \eta=||\omega ||^2\neq0~.
$$


\bthm[Poincar\'e duality]\label{thm:Poincare-duality}
For a connected, oriented, closed $n$-dimensional Riemannian manifold, the bilinear map
$$

...
\bexample
Let us consider the $n$-dimensional sphere
  \[
  ...
  \]
  where the south pole is $u^0=-1$ and the north pole is $u^0=1$, and continuous maps
  \begin{align*}
...
$$
(\varphi^\pm)^{-1}:\bR^n\to U^\pm;(x^1,\dots,x^n)\mapsto \frac{1}{1+|x|^2}(\pm(1-|x|^2),2x^1,\dots,2x^n)~.
$$
...
\bexample
A rotation of $S^n$ is a diffeomorphism.
\eexample
...
\bea
  \widetilde{\varphi}:  \pi^{-1}(U) &\to \varphi(U) \times \bR^n\\
    X_p &\mapsto (x^1(p), \cdots, x^n(p), \alpha^1, \cdots, \alpha^n),
\eea
...
A smooth map $X:M\to TM$ is called a \textbf{section} of $\pi:TM\to M$ if $\pi\cdot X=id_M$:
$$\begin{tikzcd}
TM \arrow[d,"\pi"]\\

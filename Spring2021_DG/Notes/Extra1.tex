...
\section{Complex manifolds}\label{sec:complex-manifolds}
In this appendix, we briefly introduce complex manifolds. For more details, we refer to \cite{Bouchard:2007ik}.

\bdefn[Complex manifold]\index{Complex Manifold}
Let $M$ be an $2n$-dimensional manifold. $M$ is a \textbf{complex manifold} of complex dimension $n$ if
 \begin{enumerate}
  ...
  \item  There is a homeomorphism $\varphi_\alpha: U_\alpha \to \varphi_\alpha(U_\alpha) \subseteq \bC^n$, where $\varphi(U_\alpha)$ is open in $\bC^n$.
  \item For all $\alpha, \beta$, we have $\varphi_\alpha(U_\alpha \cap U_\beta)$ is open in $\bC^n$, and the transition function
   \[
...
\bdefn[holomorphic map]\index{holomorphic map}
...
If there is the inverse map that is holomorphic, it is called a \textbf{biholomorphic} map, or $M$ and $N$ are \textbf{biholomorphic equivalent}.
...
\bexample
Let us now consider compact complex manifolds. It turns out that submanifolds of $\bC^n$ are not interesting, since a connected compact analytic submanifold of $\bC^n$ is a point. However, many compact complex manifolds can be constructed as submanifolds of projective spaces $\CP^n$. We saw that $\CP^n$ is compact; all its closed complex submanifolds are also compact. In fact, there is a theorem by Chow stating that any compact submanifold of $\CP^n$ can be realized as the zero loci $F_i(z)=0$ of a finite number of homogeneous polynomial equations in the homogeneous coordinates $z_{\mu}$. These compact complex manifolds are called \textbf{complex projective variety}. One important example is the \textbf{Fermat quintic} in $\CP^4$, given as the zero locus of the equation
\be
5\psi z_0z_1z_2z_3z_4-\sum_{\mu=0}^4 (z_{\mu})^5 = 0.
\ee
This three-dimensional compact complex manifold turns out to be \CY, probably the most studied \CY\ threefold because of mirror symmetry \cite{Candelas:1990rm}.
\eexample
...
\bdefn[Holomorphic vector bundle]\index{vector bundle}
 ...
 \begin{enumerate}
  ...
  \item For all $p \in M$, there is an open $U \subseteq M$ containing $p$ and a diffeomorphism
   \[
    t: E_U = \pi^{-1}(U) \to U \times \bC^r
   \]
   such that
   \[
    \begin{tikzcd}
     E_U \ar[r, "t"] \ar[d, "\pi"] & U \times \bC^r \ar[dl, "p_1"]\\
     U
    \end{tikzcd}
   \]
   commutes, and the induced map $E_q \to \{q\} \times \bC^r$ is a linear isomorphism for all $q \in U$.
   \item
 Suppose that $t_\alpha: E|_{U_\alpha} \to U_\alpha \times \bC^r$ and $t_\beta: E|_{U_\beta} \to U_\beta \times \bC^r$ are trivializations of $E$. Then
 \[
  t_\alpha \circ t_\beta^{-1} : (U_\alpha \cap U_\beta) \times \bC^r \to (U_\alpha \cap U_\beta) \times \bC^r
 \]
...
We call $T^{(1,0)} M$ (resp. $T^{(0,1)} M$) the \textbf{holomorphic (resp. anti-holomorphic) (co)tangent bundle}.
...
$$
 ...
 $$
It is easy to check $J^2=-\textrm{id}_{TM}$, and the eigenvalues of $J$ in $T_pM \otimes \C$ are $\pm i$. Hence, we consider $T^{(1,0)}_p M$ (resp. $T^{(0,1)}_p M$) as the eigenspace of $J_p$ with eigenvalue $i$ (resp. $-i$). $J$ is called \textbf{almost complex structure}.

 An almost complex structure $J_M:TM\to TM$ is a complex structure if for any $p\in m$ there exist an open neighborhood $p\in U\subset M$, $V\subset \bC^n$, and a continuous map $\varphi:U\to V$ such that $d\varphi\cdot J_M=J_{\bC^n}\cdot d\varphi$. If an almost complex structure $J_M$ is a complex structure, $J_M$ is called to be \textbf{integrable}. The Newlander-Nirenberg theorem states that an almost complex structure $J$ is integrable if and only if the Nijenhuis tensor vanishes:
 $$
...
On a complex manifold, the exterior derivative also admits a simple decomposition: $d = \partial + \overline{\partial}$, where we defined the operators
$$\partial: \Omega^{p,q}(M) \to \Omega^{p+1,q}(M)~,\qquad \overline{\partial}: \Omega^{p,q}(M) \to \Omega^{p,q+1}(M)~.$$
The identity $d^2=0$ implies that $\partial^2=\overline{\partial}^2=0$ and $\partial \overline{\partial}+\overline{\partial} \partial=0$.
%We can also define a real operator $d^c: \Omega^k_{\C} (M) \to \Omega^{k+1}_{\C} (M)$ by $d^c = i (\overline{\partial}-\partial)$, which satisfies
%\be
%d d^c + d^c d=0,~~~(d^c)^2=0,~~~\partial = \frac12 (d+id^c),~~~\overline{\partial} = \frac12 (d-id^c),~~~d d^c = 2i \partial \overline{\partial}.
%\ee
...
$$
where $\varphi_{i_1, \cdots ,i_p}(z)$ is holomorphic. Therefore, $\phi$ is called \textbf{a holomorphic $p$-form}. In particular, when $p=n$,
$$
K_M=\bigwedge^n T^{*(1,0)} M
$$
is called \textbf{the canonical line bundle}. Since $TM$ is isomorphic to $ T^{(1,0)} M$, the first Chern class is
$$
c_1(M)=c_1(TM)=c_1(T^{(1,0)} M)=c_1(\bigwedge^n T^{(1,0)} M)=-c_1(K_M)
$$


Now we define the analog of the de Rham cohomology groups for complex manifolds:

\bdefn
Let $M$ be a complex manifold of complex dimension $m$. As $\overline{\partial}^2=0$, we can form the complex
$$
0 \xrightarrow{\overline{\partial}} \Omega^{p,0}(M) \xrightarrow{\overline{\partial}} \Omega^{p,1}(M) \xrightarrow{\overline{\partial}} \ldots \xrightarrow{\overline{\partial}} \Omega^{p,n}(M) \xrightarrow{\overline{\partial}}0~.
$$
We define the \textbf{Dolbeault cohomology groups} $H^{p,q}_{\overline{\partial}} (M)$ of $M$ by
$$
H^{p,q}_{\overline{\partial}} (M) = {{\rm Ker}(\overline{\partial}: \Omega^{p,q}(M) \to \Omega^{p,q+1}(M)) \over {\rm Im} (\overline{\partial}: \Omega^{p,q-1}(M) \to \Omega^{p,q}(M))}.
$$
\edefn

Remark that the Dolbeault cohomology groups depend on the complex structure of $M$. Note also that we could have defined the cohomology groups using $\partial$ instead of $\overline{\partial}$, this is just a matter of convention since they are complex conjugate.

\bthm[Hodge decomposition theorem for K\"ahler manifolds]
For a compact K\"ahler manifold $M$, the complex cohomology satisfies
\begin{align}
H^r(M, \C) \cong \bigoplus_{p+q=r}H^{p,q}_{\overline{\partial}} (M) ~,\cr
H^{p,q}_{\overline{\partial}} (M) =\overline{H^{q,p}_{\overline{\partial}} (M)}
\end{align}
where $H^{p,q}_{\overline{\partial}} $
...
We now define the \textbf{Hodge numbers} to be $h^{p,q} = \dim H^{p,q}_{\overline{\partial}} (M)$. The Hodge numbers of a complex manifold are summarized in what is commonly called the \textbf{Hodge diamond}:
$$
\begin{array}{ccccc}
&&h^{n,n}&& \\
...
\bthm[Hard Lefschetz Theorem] Let $(M,\omega)$ be a K\"ahler manifold. Let $L$ be
...
\bdefn
Let $(M,J)$ be a complex manifold, and let $g$ be a Riemannian metric on $M$. We call $g$ a \textbf{Hermitian metric} if $g(v,w) = g(Jv,Jw)$ for all vector fields $v,w$ on $M$
%\item{In component notation, $g_{ab} = J_a^c J_b^d g_{cd}$;}
%\item{Using the greek indices notation, $g_{ab}=g_{\alpha \overline{\beta}}+g_{\overline{\alpha} \beta}$, that is $g_{\alpha \beta} = g_{\overline{\alpha} \overline{\beta}} = 0$.}
%\end{enumerate}
\edefn
...
Using this Hermitian metric $g$, we can define a two-form $\omega$ on $M$ called the \textbf{Hermitian form} by $\omega(v,w) = g(Jv, w)$ for all vector fields on $v,w$ on $M$. In local coordinate, it can be written as
$$
...
\bdefn
Let $(M,J)$ be a complex manifold, and $g$ a Hermitian metric on $M$, with Hermitian form $\omega$. $g$ is a \textbf{K\"ahler metric} if $d \omega= 0 $. In this case we call $\omega$ a \textbf{K\"ahler form}, and we call a complex manifold $(M,J)$ endowed with a K\"ahler metric a \textbf{K\"ahler manifold}.
\edefn
...
In addition, it can be shown that locally, the K\"ahler condition $d \omega= 0$ is equivalent to the condition $\partial_{\ell} g_{j\overline k} = \partial_{j} g_{\ell\overline k}$ and its conjugate equation $\overline\partial_{\overline{\ell}} g_{j\overline k} =\overline \partial_{\overline{k}} g_{j \overline{\ell}}$. Locally, we can always integrate these equations as,
$$g_{j\overline k}= \partial_j \overline{\partial}_{\overline k} K ( z , \overline z ) ,$$
for some function $ K ( z , \overline z )$, which is known as the \textbf{K\"ahler potential}. It is unique up to K\"ahler
...
note that the K\"ahler potential cannot be a globally defined smooth function on a compact  manifold $M$.
 ...
 Since $\omega$ is closed, it defines a Dolbeault cohomology class $[\omega] \in H^{1,1}_{\overline{\partial}} (M)$, or a de Rham cohomology class $[\omega] \in H^2_{dR} (M, \R)$. Further, the wedge product of $n$ copies of $\omega$, denoted by $\omega^n$, is proportional to the volume form of $g$. Therefore it defines a non-trivial element in both $H^{n,n}_{\overline{\partial}} (M)$ and $H^{2n}_{dR} (M,\R)$ so that $[\omega]$ must be non-trivial. However, $ \partial_j \overline{\partial}_{\overline k} K ( z , \overline z ) $ is exact and therefore zero as a cohomology class. It follows that on a compact K\"ahler manifold it is impossible to define a K\"ahler potential globally.




\bexample
Complex projective space $\CP^n$ is a K\"ahler manifold. Consider the function $u(z_0,\cdots,z_m) = \sum_{\mu=0}^n |z_\mu|^2$ where $z_\mu$, $\mu=0,\cdots,n$ are homogeneous coordinates on $\bC^{n+1} \backslash \{0\}$. Define a $(1,1)$-form $\alpha$ by $\alpha =\partial \overline\partial (\log u)$. $\alpha$ cannot be the K\"ahler form of any metric on $\bC^{n+1} \backslash \{0\}$, since it is not positive. However, if we consider the projection $\pi: \bC^{n+1} \backslash \{0\} \to \CP^n$ defined by $\pi: (z_0,\cdots,z_n) \mapsto [z_0;\cdots;z_n]$, one can show that there exists a unique positive $(1,1)$-form $\omega$ on $\CP^n$ such that $\alpha = \pi^* (\omega)$. $\omega$ is a K\"ahler form on $\CP^n$; its associated K\"ahler metric is called the \textbf{Fubini-Study metric}, and is given in components by $g_{\mu \overline{\nu}} = \partial_j \overline\partial_{\overline{k}} \log u$.
$$
...
$$
H^k_{dR} (M, \C) = \bigoplus_{j=0}^k H^{j,k-j}_{\overline{\partial}} (M).
$$
This can be shown by using the Hodge theorem of a K\"ahler manifold.

\subsection{\CY \ manifolds}

We are now ready to study \CY\ manifolds, which are a particular kind of K\"ahler manifolds.

In 1954, Calabi conjectured that a compact K\"ahler manifold $M$ with $c_1=0$ admits Ricci-flat K\"ahler metric. Yau proved the conjecture in 1976 by solving the complex Monge-Amp\'ere equation. Therefore, such a manifold is called a \textbf{\CY\ manifold}.
%
%However, many different definitions of \CY\ manifolds exist in the literature; we will review some of the most common definitions and study some relations among them. We will also investigate the properties of \CY\ manifolds and study in detail a few examples. We will end this section by quickly describing `local' \CY\ manifolds (i.e. noncompact \CY\ manifolds), which have many applications for instance in topological strings and Gromov--Witten theory.

\CY\ manifolds have been studied extensively in recent decades, particularly because of their importance in string theory \cite{yau2009calabi}. While the mathematical study of \CY\ manifolds has helped us understand compactifications of string theory, the study of string theory has led to fascinating insights in the geometry of \CY\ manifolds, for example, the study of the \CY\ moduli space and mirror symmetry. \CY\ manifolds are thus a very good example of the fruitful interactions between mathematics and physics that have been taking place in the recent decades.
...
The Hodge numbers of a \CY\ manifold satisfy a few more properties, which drastically decrease the number of undetermined Hodge numbers. We will now focus on \CY\ three-folds, that is \CY\ manifolds with complex dimension $3$, for the sake of brevity. These are the most important \CY\ manifolds in string theory applications. But most results extend straightforwardly to higher dimensional \CY\ manifolds.

The Hodge numbers of K\"ahler manifolds satisfy a Hodge star duality $h^{p,q} = h^{3-q,3-p}$ and a complex conjugation duality $h^{p,q}=h^{q,p}$. For \CY\ manifolds, there is a further duality, sometimes called \textbf{holomorphic duality}. The triviality of the canonical bundle of a \CY\ manifold $M$ implies that $h^{3,0} = 1$, i.e. the existence of a unique holomorphic volume form $\Omega$. Given a $(0,q)$ cohomology class $[\alpha]$, there is a unique $(0,3-q)$ cohomology class $[\beta]$ such that $\int_M \alpha \wedge \beta \wedge \Omega = 1$ (using Stoke's theorem). Thus $h^{0,q} = h^{0,3-q}$. Therefore, for a \CY\ manifold we have that $h^{3,0}=h^{0,3}=h^{0,0}=h^{3,3}=1$.
...
One of the fascinating property of \CY\ three-folds is that they come in mirror pairs, $(M,W)$, such that $H^{2,1} (W) \cong H^{1,1} (M)$ and $H^{1,1} (W) \cong H^{2,1} (M)$. Roughly speaking, the complex structure moduli is exchanged with the K\"ahler structure moduli. This is the basic idea behind \textbf{mirror symmetry}.
...
\end{array}
$$
The moduli space of the complex structure is studied in \S\ref{sec:moduli-Riemann}.
\eexample

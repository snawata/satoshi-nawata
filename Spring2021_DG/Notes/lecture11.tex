...
$$s_1 = \sigma_1~ , \quad s_2 = \sigma_1^2 -2\sigma_2 ~, \quad s_3 = \sigma_1^3 - 3 \sigma_1 \sigma_2 + 3 \sigma_3 ~, \cdots$$
...
\be\label{Chern-character} ch_k(E) = \frac{1}{k!}\Tr \left(-\frac{F}{2\pi i}\right)^k \in H^{2k}(M) ~. \ee
...
$$\Omega^a{}_b =\textbf{R}^a{}_{bcd} e^c\wedge e^d=R^a{}_{b\mu\nu} dx^\mu\wedge dx^\nu~.$$
...
Here we introduce other characteristic classes, such as \term{Todd classes, Hirzebruch $L$-class}, and \term{$\widehat A$-class} \cite{Eguchi:1980jx}. These classes are just defined by a different basis of invariant polynomials. However, there are very important since these classes will show up in the index theorem.
...
\be \label{Todd}Td(E) =\prod_k\frac{ x_k}{1-e^{-x_k}}=1+\frac12c_1+\frac1{12}(c_2+c_1^2)+\cdots~.\ee
